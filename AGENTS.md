...
CJQT6（版本 1.7.0）是仓颉(Cangjie)语言的 Qt6 封装库：仓颉源码（`src/`）通过 FFI 调用 C++ 桥接动态库（`cjqt6_bridge`，`native/`）包装 Qt6 API。远端仓库在 **GitCode**（`https://gitcode.com/yuan_1992/CJQT6.git`），无 GitHub Actions/CI。
