...
| 路径 | 说明 |
|------|------|
| `src/<module>/` | 仓颉封装源码（15 子包：core/widgets/gui/dialogs/menu/paint/qml/multimedia/network/sql/views/print/resource/charts/richwidgets；`process` 仅在桥接层 `native/src/process/`，仓颉封装未单独建包；`richwidgets` 为现代化富控件 Avatar/Badge/Cascader/Form/Toast/Transfer 等） |
