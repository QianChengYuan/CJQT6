...
- **CI 主流水线升级到仓颉 SDK 1.2.0，并新增 1.1.0 最低版本编译门禁**：`ci.yml` 的 `CANGJIE_VERSION` 由 `1.1.0` 升为 `1.2.0`（四平台主流水线全跑最新 STS），`ci-lint.yml` 同步升到 `1.2.0`。
  ...
  - **发布/本地工具链的 SDK 版本同步**：`scripts/sync-release-artifacts.ps1` 的 `$CangjieVersion` 默认值由 `1.1.0` 改为 `1.2.0`（该参数决定拉取 `cjqt6-bridge-v<版本>-<平台>` artifact，不同步会导致发布流程拉不到 CI 产物）；`scripts/lib/common.ps1` 的 `cjpm`/`cjcov` 兜底探测路径由 `cangjie_1.1.0` 同步为 `cangjie_1.2.0`（该硬编码本身仍是既有的待修缺陷，属本地报告不入库范围，本轮仅同步版本号）。
