...
- **CI 主流水线升级到仓颉 SDK 1.2.0，并新增 1.1.0 最低版本编译门禁**：`ci.yml` 的 `CANGJIE_VERSION` 由 `1.1.0` 升为 `1.2.0`（四平台主流水线全跑最新 STS），`ci-lint.yml` 同步升到 `1.2.0`。
  ...
  - **数据来源与验证**：objectKey/SHA 取自官网 `https://cangjie-lang.cn/download/1.2.0` 的前端数据（JS chunk 内含各包直链），四个 URL 均以 HEAD 请求核对为 `200`，`Content-Length` 与官网标称大小吻合（192.26 / 193.27 / 156.2 / 250.7 MB）；Windows 1.2.0 包的 SHA 与本机官方安装包实测值**完全一致**（交叉验证）。1.1.0 的 objectKey/SHA 亦按同法从官网数据回捞，与原脚本硬编码值一致。
  - **门禁可用性实测**：以本机仓颉 **1.1.0** 的 `cjc`/`cjpm` 实跑 `cjpm build` —— **两次复测均 success**，确认 `min-version-build` job 在 1.1.0 下确实能编过（库代码未使用 1.1.0 之后的语法）。
  - **发布/本地工具链的 SDK 版本同步**：`scripts/sync-release-artifacts.ps1` 的 `$CangjieVersion` 默认值由 `1.1.0` 改为 `1.2.0`（该参数决定拉取 `cjqt6-bridge-v<版本>-<平台>` artifact，不同步会导致发布流程拉不到 CI 产物）；`scripts/lib/common.ps1` 的 `cjpm`/`cjcov` 兜底探测路径由 `cangjie_1.1.0` 同步为 `cangjie_1.2.0`（该硬编码本身仍是 `DEFECTS.md` 登记的待修缺陷，本轮仅同步版本号）。
  - **修复 `sync-release-artifacts.ps1` 选 run 的缺陷（本轮发布实测暴露）**：该脚本此前只按 `--branch main --status success --limit 1` 取「最近一次成功的 main run」，而 **Dependabot 的自动更新 run 同样落在 main 分支且结论为 success**，于是被误选，随后 `gh run download` 因该 run 不含 bridge 产物而抛错（实测误选 run 36150451279）。现显式加 `--workflow ci.yml` 过滤；改后实测正确取到 main CI run 36150444259 并完成四平台同步。
  - **GitHub 仓库 variables 现状（已核实，无需人工操作）**：`gh api repos/QianChengYuan/CJQT6/actions/variables` 实测 `total_count = 0` —— 该仓库当前**未配置任何 Actions variables**，故 `vars.CANGJIE_SDK_URL_*` 均为空、`url-override` 不覆盖内置表，**升级后各 job 直接使用内置表的 1.2.0 下载源**。（若将来配置了这些 variable，它们会优先于内置表，届时升级 SDK 版本需一并更新。）
  - **升级 lint 工作流的风险评估（本机实测）**：1.2.0 的 `cjfmt -d src/ tools/ -c cangjie-format.toml` **零 diff**；1.2.0 的 `cjlint -f src/ -c .` 与 1.1.0 **输出完全一致**（27 条告警：26×`G.ERR.01` + 1×`G.DCL.01`，退出码均为 0），故 lint 工作流升级不会新增红灯。（顺带发现 `docs/guides/toolchain.md` 所述「白名单规则下 0 告警」与现状不符，属历史遗留，本轮未处理。）
