...
if (-not $cjpm) {
    foreach ($c in @("$env:CANGJIE_HOME\tools\bin\cjpm.exe", "$env:USERPROFILE\.cangjie\tools\bin\cjpm.exe")) {
...
if ($SkipTest -or $SkipCoverage) {
    ...
    if (-not $cjcov) {
        foreach ($c in @("$env:CANGJIE_HOME\tools\bin\cjcov.exe", "$env:USERPROFILE\.cangjie\tools\bin\cjcov.exe")) {
