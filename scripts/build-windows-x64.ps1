...
Write-Host "构建 FFI Bridge - Windows x64" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
...
if ([string]::IsNullOrEmpty($QtDir)) {
    ...
    Write-Host "示例: .\build-windows-x64.ps1 -QtDir 'C:\Qt\6.5.0\msvc2019_64'" -ForegroundColor Yellow
    exit 1
}
...
# 创建构建目录
$buildDir = "native\build-windows-x64"
...
# 验证构建产物
$artifact = "Release\cjfw_bridge.dll"
