...
cmd /c "taskkill /F /IM cjqt6.test.exe /IM std.testrunner.exe 2>nul & exit 0" | Out-Null
...
if ($RunTest) {
    Write-Host ""
    Write-Host "[test] cjpm test --coverage --exclude-tags=requires_gui_dialog,requires_gui_drag,requires_audio..." -ForegroundColor Cyan
    Set-Location $RootDir
